...
export default function AnalogInputsTab({ connection }: Props) {
  ...
  const handleAddChannel = () => {
    ...
    console.log('Add analog channel')
  }
  ...
  return (
    <div className="p-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-lg font-semibold text-white">Analog Inputs</h3>
          <p className="text-sm text-gray-400 mt-1">
            Configure analog signal mappings with scaling and offsets
          </p>
        </div>
        <button
        ...
        <table className="w-full">
          <thead className="bg-slate-800">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">Offset</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">Source Address</th>
              <th className="px-4 py-3 text-left text-xs font-medium text-gray-400 uppercase">Data Type</th>
            ...
            {channels.length === 0 ? (
              <tr>
                <td colSpan={8} className="px-4 py-8 text-center text-gray-500">
                  No analog channels configured. Click "Add Channel" to get started.
                </td>
              </tr>
            ) : (
              channels.map((channel) => (
                <tr key={channel.id} className="hover:bg-slate-800/50">
                  ...
                  <td className="px-4 py-3 text-sm text-gray-400">{channel.metadata?.units || '-'}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{channel.config?.gain || 1.0}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{channel.config?.offset || 0.0}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{channel.config?.register || channel.config?.address || '-'}</td>
                  <td className="px-4 py-3 text-sm text-gray-400">{channel.config?.dataType || 'float'}</td>
      ...
      <div className="mt-4 p-4 bg-slate-800 border border-slate-700 rounded-lg">
        <div className="text-sm text-gray-400">
          <span className="font-mono">Output = (RawValue × Gain) + Offset</span>
        </div>
      </div>
    </div>
  )
